"""
Backtest harness for the Form Agent.
...
2. For each test match, asking the Form Agent to predict the winner
...
Output:
    - evaluation/results/backtest_form_agent_YYYY-MM-DD.csv (per-match predictions)
...
def backtest_form_agent():
    ...
    # Initialize the agent on full historical data
    ...
    output_file = RESULTS_DIR / f"backtest_form_agent_{datetime.now().strftime('%Y-%m-%d')}.csv"
    results_df.to_csv(output_file, index=False)

    # Compute metrics
    valid_predictions = results_df[results_df['predicted_winner'].notna()]
    non_neutral = valid_predictions[valid_predictions['predicted_winner'] != 'neutral']
    
    if len(non_neutral) == 0:
        print(f"\n=== Backtest Results ===")
        print(f"Total test matches: {len(test_matches)}")
        print(f"Valid predictions: {len(valid_predictions)}")
        print(f"Non-neutral predictions: 0 (all neutral)")
        print(f"\nResults saved to: {output_file}")
        return results_df
    
    accuracy = non_neutral['correct'].mean()
    avg_confidence = non_neutral['confidence'].mean()
    neutral_count = len(valid_predictions) - len(non_neutral)

    print(f"\n=== Backtest Results ===")
    print(f"Total test matches: {len(test_matches)}")
    print(f"Valid predictions: {len(valid_predictions)}")
    print(f"Non-neutral predictions: {len(non_neutral)}")
    print(f"Neutral (no prediction): {neutral_count}")
    print(f"Accuracy: {accuracy:.2%}")
    print(f"Average confidence: {avg_confidence:.2f}")

    # Per-team breakdown
    team_accuracy = []
    for team in non_neutral['actual_winner'].unique():
        team_matches = non_neutral[non_neutral['actual_winner'] == team]
